...
function TelegramSection() {
  ...
  useState(() => {
    fetch("/api/account/webhook")
      ...
      .then((data) => {
        ...
        // Bot token is not returned for security; user re-enters if changing
        setInitialized(true);
      })
      .catch(() => {
        setInitialized(true);
  ...
  async function handleSave() {
    if (!botToken && !connected) {
      showError("Bot token is required");
      return;
    }
    if (!chatId) {
      showError("Chat ID is required");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setConnected(true);
        setBotToken("");
        showSuccess("Telegram settings saved. Select TELEGRAM as alert channel to receive alerts.");
      } else {
        const data = await res.json().catch(() => ({}));
  ...
  return (
    <div className="bg-[#1A0626] border border-[rgba(79,70,229,0.2)] rounded-xl p-6">
      <div className="flex items-center gap-2 mb-2">
        ...
        {connected && (
          <span className="px-2 py-0.5 text-[10px] font-medium rounded-full bg-[#10B981]/20 text-[#10B981] border border-[#10B981]/30">
            Connected
          </span>
        )}
      </div>
      <p className="text-sm text-[#94A3B8] mb-4">
        Connect a Telegram bot to receive trade alerts directly in Telegram. Create a bot via{" "}
        <a
        ...
        , then paste the bot token and your chat ID below.
      </p>
        ...
        <div className="space-y-3">
          <div>
            <label
              ...
              className="block text-sm font-medium text-[#CBD5E1] mb-1"
            >
              Bot Token {connected && "(leave empty to keep current)"}
            </label>
            <input
              id="telegramBotToken"
              type="password"
              ...
              placeholder={connected ? "********" : "123456789:ABCDefGhIjKlMnOpQrStUvWxYz"}
              className="w-full px-4 py-3 bg-[#1E293B] border border-[rgba(79,70,229,0.3)] rounded-lg text-white placeholder-[#64748B] focus:outline-none focus:ring-2 focus:ring-[#22D3EE] focus:border-transparent transition-all duration-200"
            />
          </div>
          <div>
          ...
          <div className="flex gap-3">
            <button
              onClick={handleSave}
              disabled={loading || (!botToken && !connected) || !chatId}
              className="px-6 py-2.5 text-sm font-medium text-white bg-[#4F46E5] rounded-lg hover:bg-[#6366F1] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
            >
              {loading ? "Saving..." : connected ? "Update" : "Connect"}
            </button>
            {connected && (
              <button
                onClick={handleDisconnect}
                disabled={loading}
                className="px-6 py-2.5 text-sm font-medium text-[#EF4444] border border-[rgba(239,68,68,0.3)] rounded-lg hover:bg-[rgba(239,68,68,0.1)] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
              >
                Disconnect
              </button>
            )}
