...
export function VersionControls({
  ...
  return (
    <div className="h-12 bg-[#1A0626] border-t border-[rgba(79,70,229,0.2)] px-4 flex items-center justify-between">
      {/* Left side - Save and Load */}
      <div className="flex items-center gap-2">
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-4 py-1.5 bg-[#4F46E5] text-white text-sm font-medium rounded-lg hover:bg-[#6366F1] hover:shadow-[0_0_16px_rgba(34,211,238,0.25)] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
        >
          {saving ? (
      ...
      <div className="flex items-center gap-4 text-sm">
        {/* Validation Status */}
        {hasNodes && <ValidationStatus validation={validation} />}

        {/* Separator */}
        {hasNodes && <div className="w-px h-6 bg-[rgba(79,70,229,0.3)]" />}
        ...
        {autoSaveStatus === "saving" ? (
          ...
          <span className="text-[#64748B]">No versions saved yet</span>
        )}
      </div>
    </div>
  );
