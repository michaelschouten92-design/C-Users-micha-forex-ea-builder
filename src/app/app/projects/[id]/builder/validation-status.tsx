  ...
  return (
    <div className="relative">
      ...
      {showDetails && validation.issues.length > 0 && (
        <div className="absolute bottom-full left-0 mb-2 w-80 bg-[#1E293B] rounded-lg shadow-[0_8px_24px_rgba(0,0,0,0.4)] border border-[rgba(79,70,229,0.3)] overflow-hidden">
          ...
          <div className="p-2 max-h-64 overflow-y-auto">
            {validation.issues.map((issue, idx) => (
              ...
              >
                ...
                <span className={`text-xs ${issue.type === "error" ? "text-[#FCA5A5]" : "text-[#FCD34D]"}`}>
                  {issue.message}
                </span>
              </div>
            ))}
...
function CheckItem({ label, checked, required }: { label: string; checked: boolean; required?: boolean }) {
  return (
    <div className="flex items-center gap-1.5">
      {checked ? (
        ...
        <svg className={`w-3.5 h-3.5 ${required ? "text-[#EF4444]" : "text-[#64748B]"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
        </svg>
      )}
