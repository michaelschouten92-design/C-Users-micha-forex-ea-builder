...
interface NodeToolbarProps {
  ...
  onLoadTemplate?: (preset: StrategyPreset) => void;
  hasNodes?: boolean;
  isPro?: boolean;
  onClose?: () => void;
...
export function NodeToolbar({ onDragStart, onLoadTemplate, hasNodes = false, isPro = false, onClose, settings, onSettingsChange }: NodeToolbarProps) {
  const [expandedCategories, setExpandedCategories] = useState<Set<NodeCategory>>(
    new Set()
  );
  const [confirmPreset, setConfirmPreset] = useState<StrategyPreset | null>(null);
  ...
  return (
    ...
    >
      ...
      {/* Templates Section */}
      {onLoadTemplate && (
        <div className="border-t border-[rgba(79,70,229,0.2)] p-2">
          <h3 className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wider px-2 py-2">
            Templates
          </h3>
          <div className="space-y-1.5">
            {STRATEGY_PRESETS.map((preset) => (
              <button
                ...
                onMouseDown={(e) => e.stopPropagation()}
                onPointerDown={(e) => e.stopPropagation()}
                className="w-full text-left px-3 py-2.5 rounded-lg bg-[#2A1438]/80 border border-[rgba(79,70,229,0.15)] hover:border-[rgba(79,70,229,0.3)] hover:bg-[#2A1438] transition-all duration-200 group"
              >
                <div className="text-sm font-medium text-[#C4B5FD] group-hover:text-white transition-colors">
                  {preset.name}
                </div>
      ...
      {confirmPreset && (
        ...
        >
          <div
            className="bg-[#1A0626] border border-[rgba(79,70,229,0.3)] rounded-xl p-5 max-w-sm mx-4 shadow-[0_8px_32px_rgba(0,0,0,0.5)]"
            onClick={(e) => e.stopPropagation()}
          >
            <h4 className="text-white font-semibold text-sm">Load template?</h4>
            <p className="text-[#94A3B8] text-xs mt-2">
              This will replace your current strategy with <span className="text-white font-medium">{confirmPreset.name}</span>. This action cannot be undone.
            </p>
            <div className="flex gap-2 mt-4">
              <button
                onClick={() => setConfirmPreset(null)}
                className="flex-1 px-3 py-2 text-xs font-medium text-[#94A3B8] bg-[#2A1438] border border-[rgba(79,70,229,0.2)] rounded-lg hover:text-white hover:border-[rgba(79,70,229,0.4)] transition-all"
              >
                Cancel
              </button>
              <button
                onClick={() => {
                  onLoadTemplate?.(confirmPreset);
                  setConfirmPreset(null);
                }}
                className="flex-1 px-3 py-2 text-xs font-medium text-white bg-[#4F46E5] rounded-lg hover:bg-[#6366F1] transition-all"
              >
                Load
              </button>
            </div>
