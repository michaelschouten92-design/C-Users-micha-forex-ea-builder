...
function Tooltip({ text }: { text: string }) {
  return (
    <span className="relative inline-flex ml-1 group/tooltip">
      ...
      <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 px-2.5 py-1.5 bg-[#1E293B] border border-[rgba(79,70,229,0.3)] text-[#FAFAFA] text-[10px] rounded-lg shadow-lg whitespace-normal max-w-[min(12rem,calc(100vw-2rem))] text-center pointer-events-none opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 z-50">
        {text}
      </span>
    </span>
  );
...
export function SelectField<T extends string>({
  ...
  return (
    <div>
      <label htmlFor={id} className="block text-xs font-medium text-[#FAFAFA] mb-1">
        {label}
