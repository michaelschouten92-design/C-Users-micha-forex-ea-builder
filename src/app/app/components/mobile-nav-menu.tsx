...
type NavItem = "evaluate" | "monitor" | "portfolio" | "alerts" | "settings";
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-56 bg-[#111114] border border-[rgba(255,255,255,0.10)] rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] z-50 py-2 overflow-hidden">
          ...
          <Link
            href="/app/risk"
            onClick={() => setOpen(false)}
            className={`flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${activeItem === "portfolio" ? "text-[#818CF8] bg-[rgba(79,70,229,0.08)]" : "text-[#A1A1AA] hover:text-white hover:bg-[rgba(255,255,255,0.06)]"}`}
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path
                strokeLinecap="round"
