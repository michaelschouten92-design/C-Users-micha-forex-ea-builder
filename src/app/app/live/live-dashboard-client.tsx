...
import { TradeLogPanel } from "./components/trade-log-panel";
...
// ============================================
// EQUITY CURVE CHART (Operations section)
// ============================================

function EquityCurveChart({ dailyPnl }: { dailyPnl: { date: string; pnl: number }[] }) {
...
// ============================================
// SPARKLINE (inline mini chart for SummaryCards)
// ============================================
          ...
          <div className="lg:col-span-3 space-y-4 rounded-lg bg-[#0A0118]/40 border border-[#1E293B]/40 p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {(() => {
                ...
                // Sparkline: floating P&L from heartbeat equity-balance deltas
                const floatingPnlSpark = metricsSource
                  .flatMap((ea) => ea.heartbeats ?? [])
                  .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())
                  .map((h) => h.equity);

                // Sparkline: cumulative daily P&L trend
                const cumPnlSpark: number[] = [];
                let cumSum = 0;
                for (const d of dailyPnl) {
                  cumSum += d.pnl;
                  cumPnlSpark.push(cumSum);
                }
                ...
                return (
                  <>
                    <SummaryCard
                      ...
                      sparklineData={floatingPnlSpark}
                    />
                    <SummaryCard
                      label="Total Trades"
                      value={metricsSource.reduce((sum, ea) => sum + ea.totalTrades, 0)}
                      isCurrency={false}
                      sparklineData={cumPnlSpark}
                    />
                    <SummaryCard
                      label="Open Trades"
