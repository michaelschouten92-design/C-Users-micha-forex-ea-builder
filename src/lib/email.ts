...
export async function sendRenewalReminderEmail(
  ...
  const { error } = await resend.emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: `Your AlgoStudio ${planName} plan renews in ${daysUntilRenewal} days`,
    html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0F0A1A; color: #CBD5E1; padding: 40px 20px;">
          <div style="max-width: 480px; margin: 0 auto; background-color: #1A0626; border-radius: 12px; padding: 40px; border: 1px solid rgba(79, 70, 229, 0.2);">
            <h1 style="color: #ffffff; font-size: 24px; margin: 0 0 24px 0;">Subscription renewal reminder</h1>
            <p style="margin: 0 0 16px 0; line-height: 1.6;">
              Your <strong style="color: #ffffff;">${planName}</strong> plan will automatically renew in <strong style="color: #ffffff;">${daysUntilRenewal} day${daysUntilRenewal !== 1 ? "s" : ""}</strong>.
            </p>
            <p style="margin: 0 0 24px 0; line-height: 1.6;">
              No action needed if you want to continue. If you'd like to change or cancel your plan, you can do so from your account settings.
            </p>
