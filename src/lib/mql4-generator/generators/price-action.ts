...
void ${fnName}(int tf, int lookback, int minTouches, double zonePips, double &support, double &resistance)
{
   ...
   resistance = bestResistance;
      ...
      case "order-block": {
        ...
        code.onTick.push(
          `      double bodySize = MathAbs(iClose(Symbol(), ${obTf}, i) - iOpen(Symbol(), ${obTf}, i));`
        );
        code.onTick.push(`      if(bodySize > ${varPrefix}MinSize) {`);
        code.onTick.push(
          `         if(iClose(Symbol(), ${obTf}, i) > iOpen(Symbol(), ${obTf}, i) && !${varPrefix}BullValid) {`
        );
        code.onTick.push(
          `            for(int j = i + 1; j < i + 5 && j < InpOB${index}Lookback; j++) {`
        );
        code.onTick.push(
          `               if(iClose(Symbol(), ${obTf}, j) < iOpen(Symbol(), ${obTf}, j)) {`
        );
        code.onTick.push(`                  ${varPrefix}BullHigh = iHigh(Symbol(), ${obTf}, j);`);
        code.onTick.push(`                  ${varPrefix}BullLow = iLow(Symbol(), ${obTf}, j);`);
        ...
        code.onTick.push(`         }`);
        code.onTick.push(
          `         if(iClose(Symbol(), ${obTf}, i) < iOpen(Symbol(), ${obTf}, i) && !${varPrefix}BearValid) {`
        );
        code.onTick.push(
          `            for(int j = i + 1; j < i + 5 && j < InpOB${index}Lookback; j++) {`
        );
        ...
        code.onTick.push(`                  ${varPrefix}BearHigh = iHigh(Symbol(), ${obTf}, j);`);
        code.onTick.push(`                  ${varPrefix}BearLow = iLow(Symbol(), ${obTf}, j);`);
        ...
        code.onTick.push(`}`);
        code.onTick.push(
          `${varPrefix}BuySignal = (${varPrefix}BullValid && Bid <= ${varPrefix}BullHigh && Bid >= ${varPrefix}BullLow);`
        );
        code.onTick.push(
          `${varPrefix}SellSignal = (${varPrefix}BearValid && Ask >= ${varPrefix}BearLow && Ask <= ${varPrefix}BearHigh);`
        );
        code.onTick.push("");
        break;
      }
      ...
      case "fair-value-gap": {
        ...
        code.onTick.push(`${varPrefix}BuySignal = false;`);
        code.onTick.push(`${varPrefix}SellSignal = false;`);
        code.onTick.push(`// Scan for bullish FVG`);
        code.onTick.push(
          `for(int i = 0; i < InpFVG${index}MaxAge && !${varPrefix}BuySignal; i++) {`
        );
        code.onTick.push(`   double highTwo = iHigh(Symbol(), ${fvgTf}, i + 2);`);
        code.onTick.push(`   double lowZero = iLow(Symbol(), ${fvgTf}, i);`);
        ...
        code.onTick.push(`         if(Bid <= fillLevel && Bid >= highTwo) {`);
        code.onTick.push(`            ${varPrefix}BuySignal = true;`);
        code.onTick.push(`         }`);
        code.onTick.push(`      }`);
        code.onTick.push(`   }`);
        code.onTick.push(`}`);
        code.onTick.push(`// Scan for bearish FVG`);
        code.onTick.push(
          `for(int i = 0; i < InpFVG${index}MaxAge && !${varPrefix}SellSignal; i++) {`
        );
        code.onTick.push(`   double lowTwo = iLow(Symbol(), ${fvgTf}, i + 2);`);
        code.onTick.push(`   double highZero = iHigh(Symbol(), ${fvgTf}, i);`);
        ...
        code.onTick.push(`         if(Ask >= fillLevel && Ask <= lowTwo) {`);
        code.onTick.push(`            ${varPrefix}SellSignal = true;`);
        code.onTick.push(`         }`);
        code.onTick.push(`      }`);
        code.onTick.push(`   }`);
      ...
      case "market-structure": {
        ...
        code.onTick.push(`      double high_i = iHigh(Symbol(), ${msTf}, i);`);
        code.onTick.push(`      double low_i = iLow(Symbol(), ${msTf}, i);`);
        code.onTick.push(`      for(int j = 1; j <= ${varPrefix}Str; j++) {`);
        code.onTick.push(
          `         if(iHigh(Symbol(), ${msTf}, i - j) > high_i || iHigh(Symbol(), ${msTf}, i + j) > high_i) isSwingHigh = false;`
        );
        code.onTick.push(
          `         if(iLow(Symbol(), ${msTf}, i - j) < low_i || iLow(Symbol(), ${msTf}, i + j) < low_i) isSwingLow = false;`
        );
        code.onTick.push(`      }`);
        ...
        code.onTick.push(`}`);
        code.onTick.push(`// Buy on HL in uptrend`);
        ...
        code.onTick.push(
          `   if(Bid <= ${varPrefix}SwingLow + ${varPrefix}Zone && Bid >= ${varPrefix}SwingLow)`
        );
        code.onTick.push(`      ${varPrefix}BuySignal = true;`);
        code.onTick.push(`}`);
        code.onTick.push(`// Sell on LH in downtrend`);
        ...
        code.onTick.push(
          `   if(Bid >= ${varPrefix}SwingHigh - ${varPrefix}Zone2 && Bid <= ${varPrefix}SwingHigh)`
        );
        code.onTick.push(`      ${varPrefix}SellSignal = true;`);
        code.onTick.push(`}`);
        ...
        if (ms.detectBOS) {
          ...
          code.onTick.push(
            `${varPrefix}BOS_Bull = (Bid > ${varPrefix}SwingHigh && ${varPrefix}SwingHigh > 0);`
          );
          code.onTick.push(
            `${varPrefix}BOS_Bear = (Bid < ${varPrefix}SwingLow && ${varPrefix}SwingLow < DBL_MAX && ${varPrefix}SwingLow > 0);`
          );
        }
